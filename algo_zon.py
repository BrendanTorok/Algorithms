...
class InvalidRouteError(Exception):
    def __init__(self):
        super().__init__("PickupSnapshotNode")
...
def compute_total_time(route, curr_node_id):
    # take the first instance of the route and delete the second instance
    return route[curr_node_id][1] - route[curr_node_id][0]


def node_exception_handler(node):
    if node is None:
        raise InvalidNodeError()
    if node.get_id() < 0 or node.get_timestamp() < 0:
        raise InvalidNodeError()


def invalid_route_check(node, last_time):
    node_timestamp = node.get_timestamp()
    if node_timestamp - last_time < 0:
        raise InvalidRouteError()


def detect_cyclic_route(node: 'PickupSnapshotNode'):
    # run node into node validation method
    node_exception_handler(node)
    # initialize variables
    cycle_detected = False
    cycle_node_id = 0
    route = {}
    last_time = 0
    # traverse linked list until null node is found
    while node is not None:
        # check if the next node is back in time, if it is, return error
        invalid_route_check(node, last_time)

        # initialize variables in while loop
        curr_node_id = node.get_id()
        curr_node_timestamp = node.get_timestamp()
        last_time = curr_node_timestamp

        # check if the curr_node id is already in dictionary to detect cycle
        if curr_node_id in route:
            # if curr id is in dict, append curr_node_timestamp to curr_node_id in dict
            # store cycle id, and set cycle detected to True
            route[curr_node_id].append(curr_node_timestamp)
            cycle_detected = True
            cycle_node_id = curr_node_id
            node = node.get_next()

        # if no cycle found then add node to dictionary and move to next node
        else:
            route[curr_node_id] = [curr_node_timestamp]
            node = node.get_next()

    if cycle_detected is True:
        return compute_total_time(route, cycle_node_id)

    # if no cycle is detected and null is reached, then return none
    return None
...
class TestRideSnapshotNode(TestingBase):
    ...
    def test_detect_cyclic_ride(self):
        ...
        self.test_answer("test_detect_cyclic_ride_with_cycle", result, expected_answer)
